...
void ndn_udp_face_recv(void *self, size_t param_len, void *param)
{
    msg_t msg, reply;
    ...
    case GNRC_NETAPI_MSG_TYPE_RCV:
        ...
        if (pkt->type == GNRC_NETTYPE_NDN) {
            NDN_LOG_DEBUG(
                "run over pure NDN, discard from UDP face\n");
        }
...
int ndn_udp_face_up(struct ndn_face_intf *self)
{
...
int ndn_udp_face_down(struct ndn_face_intf *self)
{
    self->state = NDN_FACE_STATE_DOWN;
    return NDN_SUCCESS;
}

void ndn_udp_face_destroy(ndn_face_intf_t *self)
{
                      ...
                      uint32_t size)
{
    ...
    if (payload == NULL) {
        ...
        // @TODO: there is no specific error code yet for this!
        return -1;
    }
    ...
    if (netif != NULL) {
        gnrc_pktsnip_t *netif_hdr =
            gnrc_netif_hdr_build(NULL, 0, NULL, 0);
